...
const imageComingSoon = 'https://via.placeholder.com/150x150?text=Countertop+Image';

const waitForReact = (callback, retries = 20, interval = 300) => {
  ...
  try {
    ...
    const CountertopCard = React.memo(({ item, isInCart, addToQuote, removeFromQuote, updateSqFt, clearSqFt, index }) => {
      ...
      return React.createElement('div', { className: 'card' },
        item.imageUrl && item.imageUrl !== imageComingSoon && React.createElement('img', {
          src: item.imageUrl,
          alt: item.colorName,
    ...
    function App() {
      ...
      const fetchPriceList = async () => {
        ...
        try {
          ...
          if (processedData.length === 0) throw new Error('No valid items from API');
          ...
          try {
            const csvResponse = await fetch('https://docs.google.com/spreadsheets/d/e/2PACX-1vRWyYuTQxC8_fKNBg9_aJiB7NMFztw6mgdhN35lo8sRL45MvncRg4D217lopZxuw39j5aJTN6TP4Elh/pub?output=csv');
            const csvText = await csvResponse.text();
            const csvData = parseCSV(csvText);
            const processedData = processData(csvData);
            if (processedData.length === 0) throw new Error('No valid items from CSV');
            setPriceData(processedData);
            localStorage.setItem('priceData', JSON.stringify(processedData));
            showToast('Loaded fallback data due to server issue', true);
          } catch (csvErr) {
            console.error('fetchPriceList CSV error:', csvErr);
            const mockData = [{
              ...
              thickness: '2cm',
              material: 'Granite',
              installedPricePerSqFt: 50 * (regionMultiplier || 1.0),
              availableSqFt: 100,
              imageUrl: imageComingSoon,
              popularity: 0.8,
              isNew: false
            }, {
              id: 'mock-granite-3cm-0',
              colorName: 'Mock Granite',
              vendorName: 'Mock Vendor',
              thickness: '3cm',
              material: 'Granite',
              installedPricePerSqFt: 55 * (regionMultiplier || 1.0),
              availableSqFt: 100,
              imageUrl: imageComingSoon,
              popularity: 0.7,
              isNew: false
            ...
            localStorage.setItem('priceData', JSON.stringify(mockData));
            showToast('Failed to load data. Using mock data.', true);
          }
        } finally {
          setIsLoading(false);
        }
      ...
      const parseCSV = (csvText) => {
        ...
        return rows.slice(1).map(row => {
          ...
          return {
            colorName: obj.colorName || 'Unknown',
            vendorName: obj.vendorName || 'Unknown',
            material: obj.material || 'Unknown',
            costSqFt: parseFloat(obj.costSqFt) || 0,
            availableSqFt: parseFloat(obj.availableSqFt) || 0,
            imageUrl: obj.imageUrl || imageComingSoon
          };
        }).filter(item => item.costSqFt > 0);
      };
