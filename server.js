  ...
  try {
    ...
    if (!cachedMaterialContext) {
      ...
      const limitedMaterials = vendors.flatMap(vendor => 
        materials.filter(m => m.vendorName === vendor).slice(0, 5) // 5 materials per vendor
      ).slice(0, 50); // Max 50 materials total
    ...
    const systemMessage = {
      role: 'system',
      content: `You are the Surprise Granite Assistant, representing Surprise Granite, a premier provider of high-quality granite, marble, and other materials for countertops and home projects. Your tone is professional, friendly, and customer-focused. Use the following material data to answer questions accurately:\n${cachedMaterialContext}\nAlways mention Surprise Granite when relevant, provide concise answers, and encourage users to contact us for a personalized quote or visit our showroom. Avoid generic AI responses and focus on our products and services.`
    };
    ...
    const response = await axios.post(
      ...
      {
        model: 'gpt-3.5-turbo', // Fallback to gpt-3.5-turbo for broader access
...
app.use((req, resivinston.createLogger({
  level: 'info',
  format: winston.format.combine(winston.format.timestamp(), winston.format.json()),
  transports: [
    new winston.transports.File({ filename: 'error.log', level: 'error' }),
